...
public class GUI11 {
	 ...
	 */
	public GUI11(Kupac kupac) {
		initialize(kupac);
		table_update_nc(kupac);
		GUI1.konektuj();
	}
	
	public void table_update_nc(Kupac kupac) {
		...
		try {
			...
			while(GUI1.rs.next()) {
				...
				lista_narudzbi_na_cekanju.add(narudzba);
			}
		}
		catch(Exception ex) {
		...
		for(Narudzba n: lista_narudzbi_na_cekanju) {
			Vector v=new Vector();
			for(int a=1; a<=6; a++) {
				v.add(n.getId());
				v.add(n.getKupacId());
	...
	public void table_update_an(int id, Kupac kupac) {
		...
		try {
			...
			while(GUI1.rs.next()) {
				...
				int nid=Integer.parseInt(GUI1.rs.getString(5));
				ArtikalNarudzbe artikal_narudzbe=new ArtikalNarudzbe(narudzba_id, proizvod_id, kolicina, cijena_po_komadu, nid);
	...
	private void initialize(Kupac kupac) {
		...
		JLabel lblNewLabel = new JLabel("Narudzbe na cekanju");
		lblNewLabel.setFont(new Font("Tahoma", Font.BOLD, 20));
		lblNewLabel.setHorizontalAlignment(SwingConstants.CENTER);
		lblNewLabel.setBounds(242, 11, 272, 31);
		frame.getContentPane().add(lblNewLabel);
		
		lblNewLabel_12 = new JLabel("Prodajno mjesto");
		...
		table_1.setModel(new DefaultTableModel(
			...
			new String[] {
				"Narudzba ID", "Proizvod ID", "Kolicina", "Cijena", "ID"
			}
		) {
			Class[] columnTypes = new Class[] {
		...
		btnNewButton = new JButton("Otkazi");
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					GUI1.pst=GUI1.con.prepareStatement("delete from artikal_narudzbe where narudzba_id=?");
				...
				try {
					int id=Integer.parseInt(GUI1.df.getValueAt(selectedIndex, 0).toString());
					GUI1.pst=GUI1.con.prepareStatement("delete from narudzba where id=?");
					GUI1.pst.setInt(1, id);
					GUI1.pst.executeUpdate();
					JOptionPane.showMessageDialog(null, "Narudzba je otkazana.");
					table_update_nc(kupac);
					table_update_an(id, kupac);
				}
				catch(SQLException ex) {
		...
		btnNewButton.setBounds(28, 328, 89, 23);
		...
		btnNewButton_1.setBounds(28, 358, 89, 23);
